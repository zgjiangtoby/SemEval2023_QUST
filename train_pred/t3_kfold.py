...
for fold, (train_index, test_index) in enumerate(K_fold.split(df['text'].values, df['id'].values)):
    print(f"Fold {fold + 1}:")
    ...
    train_datasets = train_datasets.remove_columns(["text", "__index_level_0__","para_id"])
    ...
    val_datasets = val_datasets.remove_columns(["text", "__index_level_0__","para_id" ])
    ...
    if from_local:
        model = XLMRobertaForSequenceClassification.from_pretrained("./checkpoints/t2/t2_fold_10.model", num_labels=24,
                                                                    problem_type="multi_label_classification",
                                                                    ignore_mismatched_sizes=True)

    else:
        model = XLMRobertaForSequenceClassification.from_pretrained("xlm-roberta-base", num_labels=24,
                                                                    problem_type="multi_label_classification")
    ...
    for epoch in range(num_epochs):
        ...
        if epoch - saved_epoch >= tolerant:
            ...
            print(fold_val_f1)
            print(max_f1)
            with open("./val_finetune_t3_fold{}.txt".format(fold + 1), 'w') as outf:
